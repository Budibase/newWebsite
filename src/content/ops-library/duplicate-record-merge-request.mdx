---
slug: duplicate-record-merge-request
title: Duplicate Record Merge Request
sidebar:
  - id: overview
    label: Overview
  - id: data
  ...
  - id: automations
    label: Automations
role: Rev Ops
outcome: Triage duplicate CRM records, compare survivorship rules, and merge safe duplicates with audit visibility.
setupTime: "~45 min"
difficulty: Medium
...
steps:
  - title: Request
    description: A requester submits a duplicate record merge request through Slack, Teams, or the Duplicate Merge Request Form, creating a tracked request row.
    humanDecision: true
  - title: Agent Triage
    description: The agent confirms the request type, retrieves context from CRM records and data quality rules, checks the relevant policy source, and asks for any missing details in chat.
    humanDecision: false
  - title: Agent Resolution
    description: Where policy permits, the agent completes duplicates with clear primary record, matching identifiers, and no ownership conflict and records the outcome without waiting for manual handling.
    humanDecision: false
  - title: Escalation
    description: The agent escalates conflicting account ownership, open opportunities on both records, or uncertain survivorship to the responsible owner with a decision summary and supporting context.
    humanDecision: false
  - title: Human Resolution
    description: An approver reviews the escalation in chat or the Data Quality Review Queue, then approves, rejects, or returns it for more information.
    humanDecision: true
integrations:
  - Slack
  - Teams
...
faq:
  - question: Can the agent complete requests without approval?
    answer: Yes. It can auto-resolve duplicates with clear primary record, matching identifiers, and no ownership conflict when the policy source allows it and the data is complete.
  - question: When does a human get involved?
    answer: Human review is used for conflicting account ownership, open opportunities on both records, or uncertain survivorship.
  - question: Where is the audit trail kept?
    answer: The request table stores current state, while the Duplicate Record Merge Audit Log records each decision, message, tool action, and handoff.
...
<OpsLibraryAppDemo
  appName="Duplicate Record Merge Request"
  inboxRows={[
    {
      id: "DQ-1048",
      request: "Duplicate Record Merge Request for Emma Clarke",
      priority: "High",
      status: "Needs Review",
      date: "May 18",
      tone: "high",
