...
sidebar:
  ...
  - id: automations
    label: Automations
role: IT Ops
outcome: Track leaver requests, schedule access removal, confirm asset return, and escalate risky offboarding exceptions.
setupTime: "~45 min"
difficulty: Medium
...
steps:
  - title: Request
    description: A requester submits a leaver offboarding through Slack, Teams, or the Leaver Offboarding Form, creating a tracked request row.
    humanDecision: true
  - title: Agent Triage
    description: The agent confirms the request type, retrieves context from HRIS, identity provider, and ITSM records, checks the relevant policy source, and asks for any missing details in chat.
    humanDecision: false
  - title: Agent Resolution
    description: Where policy permits, the agent completes standard access suspension and checklist creation when the leaver record is complete and records the outcome without waiting for manual handling.
    humanDecision: false
  - title: Escalation
    description: The agent escalates executive leavers, immediate terminations, missing dates, or unreturned assets to the responsible owner with a decision summary and supporting context.
    humanDecision: false
  - title: Human Resolution
    description: An approver reviews the escalation in chat or the Offboarding Exceptions Queue, then approves, rejects, or returns it for more information.
    humanDecision: true
integrations:
  - Slack
  - Teams
...
faq:
  - question: Can the agent complete requests without approval?
    answer: Yes. It can auto-resolve standard access suspension and checklist creation when the leaver record is complete when the policy source allows it and the data is complete.
  - question: When does a human get involved?
    answer: Human review is used for executive leavers, immediate terminations, missing dates, or unreturned assets.
  - question: Where is the audit trail kept?
    answer: The request table stores current state, while the Leaver Offboarding Audit Log records each decision, message, tool action, and handoff.
...
heroPrompt: |
  # Agent role
  You are the Leaver Offboarding agent. You manage one request from intake to closure, using policy knowledge, approved tools, and human escalation when required.
  ...
  # Tool use
  - Use {{ budibase.Leaver Offboarding Requests.get_row }} and {{ budibase.Leaver Offboarding Requests.update_row }} to maintain request state.
  - Use {{ budibase.Leaver Offboarding Audit Log.create_row }} to log decisions and actions.
  - Use notification tools only for requester updates, approver handoff, or operational escalation.

  # Auto-resolution
  - Use {{ bambooHR.get_employee }} to verify the leaver record, termination date, manager, and employment type.
  - If the offboarding policy allows standard processing, use {{ okta.suspend_user }} at the approved time.
  - Use {{ okta.revoke_user_sessions }} to invalidate active sessions after suspension.
  - Use {{ jira.create_issue }} to create asset return or handover tasks for IT owners.
  - Escalate immediate terminations, executives, legal holds, and missing HR data before taking access actions.
...
SEC-1048,emma.clarke@example.com,"Please process this leaver offboarding.","Example target",Escalated,High,Escalated,"Requires owner approval.",2026-05-18T09:15:00.000Z,
```

**Leaver Offboarding Audit Log**: Records agent decisions, tool calls, escalations, notifications, and human actions.
...
EVT-2048,SEC-1048,Leaver Offboarding Agent,Escalation,"{""reason"":""Policy requires owner review""}",2026-05-18T09:18:00.000Z
```
...
<OpsLibraryAppDemo
  appName="Leaver Offboarding"
  inboxRows={[
    {
      id: "SEC-1048",
      request: "Leaver Offboarding for Emma Clarke",
      priority: "High",
      status: "Needs Review",
      date: "May 18",
      tone: "high",
